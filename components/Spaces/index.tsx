//types
import type { SubscribedPublication, PinnedItem } from 'contexts'
//data
...
interface ISpace {
    ...
    currentSpace: string;
    setCurated: (fn: (prevState: SubscribedPublication[]) => SubscribedPublication[]) => void;
}
                ...
                <ButtonPopover isHighlighted={false} icon={<DeleteIcon />} label='Delete'>
                    <Box layout='flexBoxColumn' css={{ color: '$foregroundTextBronze' }}>
                        ...
                        <StyledButton onClick={() => {
                            setCurated((prevState: SubscribedPublication[]) => {
                                const indexUnPin = prevState.findIndex((item: SubscribedPublication) => item.displayName === currentSpace)
